...
> **来源**：从 info-collector 的 `references/writing-guide.md` 提炼的通用部分。
> **状态**：独立于任何 research skill。任何调研 agent 读到即可参考。
...
- 比较用表格，不是段落
- 每个 claim 在内容中紧邻其来源引用

## Source Traceability

- **每个量化声明（基准数字、百分比等）必须带来源标识**：URL 或引用编号
- **基准数据必须包含测试环境摘要**：至少硬件、OS、运行时版本、测试日期
- **报告中每个来源名都要有对应的完整可点击 URL**

### Tier-aware 引用语言

不同来源层级需要不同的正文措辞，准确反映证据权重：

| 来源层级 | 引用语言规则 | 示例 |
|---|---|---|
| Tier 1（官方文档、标准组织） | 权威引用 | "根据 OpenAI 官方文档..." |
| Tier 2（行业报告、主流媒体） | 带来源归属 | "2025 年 Gartner 报告估计..." |
| Tier 3（博客、教程、社区帖） | 用限定词 | "一篇博客指出"、"社区分析表明" |
| Tier 4（论坛、个人页面、未验证） | 用强限定词 | "一个未验证来源声称"、"一篇论坛帖提到" |
...
- 不同测试条件下的基准数字 → 用 `precision: range`，在 `source_metadata.test_conditions` 注明
...
- ❌ **来源与 claim 分离** — 来源 URL 必须紧邻其 claim，不要堆在末尾
- ❌ **伪综合** — 用因果/矛盾语言（"核心矛盾是"、"综合来看"、"本质上"）但无因果证据。若 A 和 B 共现但无法用来源建立 A→B，写"A 和 B 共现；是否有因果关系，现有来源未建立"
- ❌ **名字当分析** — 提及实体 + 一句话描述，但不评估其意义或与替代方案对比。每节至少有 ≥2 个分析性条目（带评估、对比或影响判断，不只是存在性陈述）
- ❌ **行动陈词滥调** — "读者需要理解 X"或"工程师应该意识到 Y"但无具体、来源支持的指引。要么给具体行动项（"优先 X 因为 Y，来源支持 [src]"），要么删掉这段
