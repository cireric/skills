...
### R1: `covered_directions` 最大长度硬编码

| 位置 | 代码 |
|------|------|
| `schemas.py:222` | `if len(cd) > 3:` |
| `proceed.py:205` | `if len(cd) > 3:` |

同一业务规则在两处硬编码。应提取为 `constants.py` 中的 `_MAX_COVERED_DIRECTIONS = 3`。

### R2: fallback route 字典硬编码
...
应提取为模块级常量 `_DEFAULT_ROUTE = {"entry_tier": 3, "path": [3, 2, 1]}`。

### R3: `defined_nums | visible_nums` 合并逻辑
...
R3+R4 应提取为两个辅助函数：`_extract_defined_nums(ref_section)` 和 `_extract_cited_nums(body)`。

### R5: `_source_text()` 内联重复

| 位置 | 代码 |
|------|------|
| `artifact_checks.py:44-45` | `def _source_text(item): return (item.get("fetched_content", "") + " " + item.get("snippet", "")).lower()` |
| `artifact_checks.py:770-772` | `(item.get("fetched_content", "") + " " + item.get("snippet", "")).lower()` |

`check_claim_source_relevance` 内联了与 `_source_text()` 完全相同的拼接逻辑。

### R6: collected URL 集合构建
...
两者均从 collected.json 构建 `normalize_url` 集合，逻辑重复。
...
`artifact_checks.py`（818 行）承载 17 个 check 函数 + 辅助函数。已拆出 `report_checks.py`，但 artifact 侧仍臃肿。
...
`cli.py:191` 在 `_build_report_filename` 内 `import re as _re`，顶层已有 `import re`。应直接使用顶层 `re`。
...
### S6: `_PHASE_ARTIFACTS` 不完整

`constants.py:149-154` 仅含 scope/search/analysis/review 四个 key，pipeline 实际还有 final 和 cleanup 阶段。`cmd_reset` 无法重置到 final 阶段。

### S7: `_read_artifact` level 参数语义不清
...
`level` 参数控制"读不到时的宽容度"，返回值 level 是"检查结果严重度"——两者语义不同但共用 `level` 命名，容易误读为"这个 check 永远是 WARN 级别"。

### S8: `_gate_analysis` 只检查 `url_traceability`

`proceed.py:428-432`：调用 `run_all`（17 个 check）但仅用 `next()` 提取 `url_traceability` 一个结果。其余 BLOCKER（如 `precision_inflation`、`source_metadata`）被静默忽略。详见逻辑谬误 L2。
...
**修复**: 按方向单独判断 CJK，只对含 CJK 字符的未覆盖方向降级：
...
### L2: `_gate_analysis` 遗漏 BLOCKER ⚠️

**文件**: `proceed.py:428-432`
...
### L3: `_gate_review` 全量重跑 gateway ⚠️

**文件**: `proceed.py:436-439`
...
**建议**: 缩小检查范围至 review-specific check，或在文档中明确全量运行的意图。

### L4: `_generate_search_plan` 语言判断粒度不足
...
| 编号 | 对应问题 | 修复要点 |
|------|----------|----------|
| P0-1 | L1 | `_check_topic_coverage` 中按方向单独判断 CJK，只对含 CJK 的 missing 方向降级为 WARN |
| P0-2 | L2 | `_gate_analysis` 中检查 `run_all` 返回的全部 BLOCKER，而非仅 `url_traceability` |

### P1 — 应该修复（重复定义 + 代码异味）

| 编号 | 对应问题 | 修复要点 |
|------|----------|----------|
| P1-1 | R1 | `constants.py` 添加 `_MAX_COVERED_DIRECTIONS = 3`，替换 `schemas.py` 和 `proceed.py` 中硬编码 |
| P1-2 | R2 | `source_router.py` 提取 `_DEFAULT_ROUTE` 常量，替换两处硬编码 fallback |
| P1-3 | R3+R4 | `report_checks.py` 提取 `_extract_defined_nums()` 和 `_extract_cited_nums()` 辅助函数 |
| P1-4 | R5 | `artifact_checks.py:770-772` 用 `_source_text(item)` 替换内联拼接 |
| P1-5 | R6 | 提取 `build_collected_url_set(collected)` 到 `utils.py`，统一两处集合构建 |
| P1-6 | S3 | `cli.py` 删除 `import re as _re`，直接使用顶层 `re` |
| P1-7 | S4 | `_find_report_path` 添加 `config` 参数，避免重复读 config.json |
| P1-8 | S7 | 重命名 `_read_artifact` 的 `level` 参数为 `read_error_level`，消除与返回值 `level` 的语义混淆 |

### P2 — 建议改进（架构 + 可读性）

| 编号 | 对应问题 | 修复要点 |
|------|----------|----------|
| P2-1 | S1 | 拆分 `artifact_checks.py` 为 `claim_checks.py` / `coverage_checks.py` / `content_checks.py` |
| P2-2 | S5 | 去掉 `constants.py` 中被跨模块使用的常量下划线前缀，或提供公开访问函数 |
| P2-3 | S2 | 将 gate 逻辑独立为 `gates.py`，`proceed.py` 只负责状态转换和 gate 调度，解除循环依赖 |
| P2-4 | S6 | `_PHASE_ARTIFACTS` 补充 `final` 和 `cleanup` key |
| P2-5 | S9 | `_VALID_TRANSITIONS_SET` 改用 dict 映射 `(from, to) → gate_fn`，消除 `proceeds()` 中的 `gate_fn` 字典 |
| P2-6 | S10 | `_detect_quality` 用 dict mapping 替代多分支 if/elif |
| P2-7 | L3 | `_gate_review` 缩小检查范围至 review-specific check |
| P2-8 | L4 | `_generate_search_plan` 语言判断从 tier 级下推到 source 级 |
