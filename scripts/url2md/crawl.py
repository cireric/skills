...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("url", nargs="*", help="Article, list page URL, or multiple URLs (optional with --preflight)")
...
def main() -> None:
    ...
    if args.filename and len(args.url) > 1:
        parser.error("--filename 仅支持单个 URL 时使用")
    ...
    # 多 URL 时禁用 config 的单一 filename，避免各 URL 覆写同一文件
    filename = args.filename or (config.get("filename") if len(args.url) == 1 else None)
    ...
    for url in args.url:
        ...
        if result.success:
            ...
            if result.article_count > 1:
                print(f"已抓取 {result.article_count} 篇文章: {url}")
            ...
            failures.append(f"{url}: {result.error}")
            print(f"抓取失败: {url}: {result.error}")

    if failures and not all_files:
        sys.exit(1)

    if all_files:
        print(json.dumps(all_files, ensure_ascii=False))
