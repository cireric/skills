...
- mark/archive snapshot the full log.md to log.md.bak-YYYY-MM-DD before
  rewriting; this is a crash-rollback point, overwritten per-day and pruned
  to the newest `backup_retain_count` (default 2). Resolved entries also live
  on permanently in archive/log-YYYY-MM-DD.md, so stale .bak files are safe
  to delete.
"""
...
def _prune_backups() -> None:
    retain = int(_cfg("backup_retain_count", 2))
    pattern = re.compile(r"^" + re.escape(LOG_FILENAME) + r"\.bak-(\d{4}-\d{2}-\d{2})$")
    backups = sorted(
        (p for p in obs_dir().glob(f"{LOG_FILENAME}.bak-*") if pattern.match(p.name)),
        key=lambda p: p.name,
        reverse=True,
    )
