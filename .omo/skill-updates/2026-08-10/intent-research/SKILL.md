...
**Windows 编码说明**: CLI 入口已强制 stdout/stderr 为 UTF-8（`errors="replace"`），`verify` 输出的 ‡/† 标记在 GBK 控制台下也不会崩溃。若直接调用 `scripts/cli.py` 而非 `-m scripts.cli`，请自行加 `PYTHONIOENCODING=utf-8`。
